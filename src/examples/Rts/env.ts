...
export class Env {
	...
	psize: number = 30
	hyperparams: Hyperparameters = new Hyperparameters()
	neat: Neat = new Neat(4, 4, this.psize, this.hyperparams).import();

	constructor(p: p5) {
		this.p = p
		this.hyperparams.default_activation = activation.TANH
	...
	draw = () => {
		...
		// 获取家的单位
		const home = this.unit.filter((x) => x.unitName == 'home')[0]

		// 获取测试坦克
		const taskList = this.unit.filter((x) => x.unitName == 'testTank')

		// 循环AI的操作
		for (let i = 0, len = this.neat.get_genomes().length; i < len; i++) {
			const take = taskList[i]
			const desicions = this.neat.get_genomes()[i].forward([
				take.x,
				take.y,
				home.x,
				home.y
		...
		const fullDead = taskList.map((x) => x.blood).every((x) => x <= 0)
		...
		if (fullDead) {
			...
			for (let i = 0, len = this.neat.get_genomes().length; i < len; i++) {
				...
				// 使用距离评分
				const dx = Math.abs(take.x - home.x);
				const dy = Math.abs(take.y - home.y);
				const num = Math.sqrt(Math.pow(dx, 2) + Math.pow(dy, 2));
				// console.log(num)

				this.neat.get_genomes()[i].set_fitness(1000 - num)

				// 重置

				take.x = getRandomInt(0, this.p?.height ?? 0)
				take.y = getRandomInt(0, this.p?.height ?? 0)
				take.blood = take.maxBlood
				take.currentIndex = take.path.length
			}
			// console.log(taskList)

			if (this.neat.should_evolve()) {
				console.log(this.neat)
				this.neat.next_iteration();
			}
		}
