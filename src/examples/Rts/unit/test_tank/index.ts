...
export class TestTank {
	...
	aiMove(val: number) {
		if (this.blood <= 0 || this.isMove == true) return

		if (val == 0) {
			this.setPath(this.x + this.env.gridSize * 2, this.y)
		}
		if (val == 1) {
			this.setPath(this.x - this.env.gridSize * 1, this.y)
		}

		if (val == 2) {
			this.setPath(this.x, this.y + this.env.gridSize * 2)
		}

		if (val == 3) {
			// this.setPath(this.x,this.y-1)
			this.setPath(this.x, this.y - this.env.gridSize * 1)
		}
